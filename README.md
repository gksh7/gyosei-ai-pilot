...
- [x] A8アフィリエイトリンク自動挿入（2026年5月11日）
  ...
  - A8承認済み8件登録（KANBEI SIGN・弥生会計 Next・MFクラウド会計・MF会社設立・アガルート行政書士講座・行政書士TEPPAN通信講座・資格スクエア・SMART合格講座）
  - アガルート行政書士講座は `categories:[]`（汎用枠）として全記事に必ず1枠表示
  - 残り承認待ちは is_active=true + a8_link 更新で即反映

### Phase 8：アフィリエイト運用の高度化（2026年6月〜7月・完了）

- [x] **アガルート行政書士講座の訴求強化** — 記事詳細・トップページの全アフィリエイト枠で常に先頭表示に固定、PC版表示を斜め矢印アイコンに統一、記事本文内インラインバナーは削除しサイドバー表示に一本化
- [x] **アフィリエイト入れ替え** — ベクターサイン（クラウド型電子契約サービス・株式会社ベクターホールディングス）を追加登録（2026年6月5日）→ A8プロモーション終了に伴い `is_active=false` で無効化（2026年7月11日・履歴として保持）
- [x] **GSCタイトル改善の適用運用を確立** — `gsc_suggestions` の日次メール通知を人が確認・採用案を判断（Human in the loop）→ `scripts/apply-gsc-titles.ts` で記事タイトルを更新 → Indexing APIで再クロール通知
...
### Phase 6：Instagram自動投稿（2026年5月16日実装・2026年6月6日無効化）

- [x] **Instagram Graph API 連携**（`scripts/instagram-poster.ts`）
  - 毎朝の記事生成後、OGP画像をフィード投稿として自動公開していた
  - **2026年6月6日にパイプラインから除外**
...
[@gyosei_ai_pilot](https://www.instagram.com/gyosei_ai_pilot/) — 自動投稿停止中（2026年6月6日に無効化）
...
1. アフィリエイト強化
2. 行政書士つなぎ役ビジネス：診断結果ページに「専門家に相談する」ボタンを置き、提携行政書士に問い合わせフォームを流す。成約報酬型（例：1件3,000〜5,000円）で行政書士側と直接契約。
